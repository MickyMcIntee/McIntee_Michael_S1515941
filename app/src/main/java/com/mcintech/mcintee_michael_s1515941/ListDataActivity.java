...
public class ListDataActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(holder.getChildCount() == 0) {
            Button b = new Button(this);        //Create a button in the context of list data activity.
            b.setText("No Data found, check connection and click here to force update.");              //Set the text of the button to the current items description.
            LinearLayout.LayoutParams param = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
            param.setMargins(8,5,8,5);
            b.setLayoutParams(param);                   //Apply the above layout parameters to the buttons programmatically.
            b.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 20); //Set size of the button text to Dp 20
        ...
        idd = new ImageDownloader(); //Create a new image downloader object which is a form of a runnable. Nested class can be found further in code.
    ...
    private class ImageDownloader implements Runnable {
        private Channel channel;    //Link to the file to download.

        /**
         * Constructor which accepts the link and sets the link of the objects value to the passed in
         * parameter.
         *
         * v2.0 Changed to use a reference of the outer classes channel to ensure that the app
         * can function with no data and the image is loaded when the data is.
         */
        public ImageDownloader() {
            this.channel =  ListDataActivity.getChannel();
        }
         ...
         *
         * v2.0 made channel static and use it to ensure that the channel is updated
         * after each check so the image can be downloaded if the connection is made
         * again.
         */
        public void run() {

            //Wait until the channel has items.
            while(channel.getImage() == null) {
                try {
                    Thread.sleep(5000);
                    this.channel = ListDataActivity.getChannel();
                } catch (InterruptedException e) {
            ...
            image = loadImage(channel.getImage().getURL());
        }
